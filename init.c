...
/*   Updated: 2022/03/29 12:27:06 by algabrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pthred	**info_inicialize(t_philo *data)
{
	...
	while (i < data->number_of_philosophers)
	{
		...
		info[i]->left_fork = data->forks[i];
		info[i]->right_fork = data->forks[i + 1];
		i++;
	}
	if (1 < data->number_of_philosophers)
		info[data->number_of_philosophers - 1]->right_fork = data->forks[0];
	return (info);
}
